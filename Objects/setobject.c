...
set_dealloc(PyObject *self)
{
    ...
    if (so->table != so->smalltable)
        PyMem_Free(so->table);
    PyType_Call_tp_free(Py_TYPE(so), so);
}
...
static PyMethodDef setiter_methods[] = {
    {"__length_hint__", setiter_len, METH_NOARGS|METH_C_STACK_FRUGAL, length_hint_doc},
    {"__reduce__", setiter_reduce, METH_NOARGS|METH_C_STACK_FRUGAL, reduce_doc},
    {NULL,              NULL}           /* sentinel */
};
...
make_new_set(PyTypeObject *type, PyObject *iterable)
{
    ...
    so = (PySetObject *)PyType_Call_tp_alloc(type, type, 0);
    if (so == NULL)
        return NULL;
