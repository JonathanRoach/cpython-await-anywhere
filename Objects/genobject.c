...
static void
_gen_dealloc_outer(PyObject *self, void(*inner_dealloc)(PyGenObject *))
{
    PyGenObject *gen = _PyGen_CAST(self);
...
{
    _gen_dealloc_outer(self, &_gen_dealloc_inner);
...
{
    if (sendres == PYGEN_RETURN) {
        if (PyAsyncGen_CheckExact(op)) {
...
gen_close(PyObject *self, PyObject *args)
{
    ...
    if ((PyCoro_CheckExact(gen) || PyAsyncGen_CheckExact(gen)) && ((PyCoroObject*)self)->cr_coroutine) {
        ...
        if (coro->cr_frame_state == FRAME_EXECUTING)
        {
            ...
            PySendResult sendresult = coro_dosend(coro, NULL, true, true);
...
{
    PyCoroObject *coro = _PyCoroObject_CAST(gen);
...
coro_dosend(PyCoroObject *coro, PyObject *val, int exc, int closing)
{
    if (!coro->cr_coroutine){
        if (FRAME_STATE_FINISHED(coro->cr_frame_state)) {
            if (PyCoro_CheckExact((PyObject *)coro) && !closing) {
                /* `gen` is an exhausted coroutine: raise an error,
                except when called from gen_close(), which should
                always be a silent method. */
                PyErr_SetString(
                ...
                /* `gen` is an exhausted generator:
                only return value if called from send(). */
                coro->cr_result = Py_NewRef(Py_None);
                return PYGEN_RETURN;
            }
            coro->cr_result = NULL;
            return PYGEN_ERROR;
    ...
    coro->cr_py_recursion_depth_at_entry = tstate->py_recursion_limit - tstate->py_recursion_remaining;
    ...
    tstate->py_recursion_remaining = tstate->py_recursion_limit - coro->cr_py_recursion_depth_at_entry;
...
_PyCoro_DoYield(PyObject *op, PyObject *yield_from)
{
    ...
    if (!coro || !coro->cr_coroutine){
        ...
        PyErr_SetString(
            PyExc_RuntimeError,
            "can only yield the running coroutine");
        return NULL;
    }

    // The following are recorded for restoration with ThreadState when resuming after yield
    PyThreadState *tstate = _PyThreadState_GET();
    int py_recusion_depth_for_coroutine = tstate->py_recursion_limit - tstate->py_recursion_remaining - coro->cr_py_recursion_depth_at_entry;
    _PyInterpreterFrame *current_frame = tstate->current_frame;
    int recursion_headroom = tstate->recursion_headroom;
    _PyErr_StackItem *exc_info = tstate->exc_info;
    _PyDataStack *datastack = tstate->active_datastack;
...
"yield() -> yield the current coroutine.");
...
static void
async_gen_dealloc(PyObject *self)
{
...
{
    // this check is to cover async generator tests which call .send() directly, ourside of a Task
    // and expect the old behaviour
    if (with_unwrap){
        result = async_gen_unwrap_value(gen, result);
    }
    if (coro_active){
        if (result && !_PyAsyncGenWrappedValue_CheckExact(result)){
            PySendResult sendresult;
            do {
                PyObject *r = _PyCoro_DoYield(result, NULL);
                Py_DECREF(result);
                result = NULL;
                sendresult = coro_dosend((PyCoroObject *)gen, r, r == NULL, false);
                Py_XDECREF(r);
                result = gen_to_return((PyObject *)gen, sendresult, gen->ag_result);
                if (with_unwrap){
                    result = async_gen_unwrap_value(gen, result);
                }
            } while (sendresult == PYGEN_NEXT && result && !_PyAsyncGenWrappedValue_CheckExact(result));
        }
    }
    return result;
}
