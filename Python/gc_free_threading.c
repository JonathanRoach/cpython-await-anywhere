            ...
            void *block, size_t block_size, void *args)
{
    ...
    // collected.
    PyType_Call_tp_traverse(Py_TYPE(op), op, visit_decref, NULL);
    return true;
}
...
mark_reachable(PyObject *op)
{
    ...
    do {
        if (PyType_Call_tp_traverse(Py_TYPE(op), op, visit_clear_unreachable, &stack) < 0) {
...
gc_propagate_alive_prefetch(gc_mark_args_t *args)
{
    for (;;) {
        ...
        else if (PyType_Call_tp_traverse(Py_TYPE(op), op, gc_mark_enqueue_buffer_visitproc, args) < 0) {
            return -1;
        }
    }
}
...
gc_propagate_alive(gc_mark_args_t *args)
{
    if (args->use_prefetch) {
        ...
        for (;;) {
            ...
            assert(_PyObject_GC_IS_TRACKED(op));
            assert(gc_is_alive(op));
            if (PyType_Call_tp_traverse(Py_TYPE(op), op, gc_mark_enqueue_no_buffer_visitproc, args) < 0) {
                return -1;
            }
        }
        return 0;
...
handle_resurrected_objects(struct collection_state *state)
{
    ...
    WORKSTACK_FOR_EACH_ITER(&state->unreachable, &iter, op) {
        ...
        (void)PyType_Call_tp_traverse(Py_TYPE(op), op, visit_decref_unreachable, NULL);
    }
                    ...
                    void *block, size_t block_size, void *args)
{
    ...
    if (PyType_Call_tp_traverse(Py_TYPE(op), op, referrersvisit, arg->objs)) {
