...
{
    _PY_ENSURE_COSTACK_HEADROOM_FOR_FN3_B((PyErr_NoMemory(), 0), int, astfold_expr, node_, ctx_, state)
    ENTER_RECURSIVE();
    switch (node_->kind) {
    case BoolOp_kind:
...
{
    _PY_ENSURE_COSTACK_HEADROOM_FOR_FN3_B((PyErr_NoMemory(), 0), int, astfold_stmt, node_, ctx_, state)
    ENTER_RECURSIVE();
    switch (node_->kind) {
...
astfold_pattern(pattern_ty node_, PyArena *ctx_, _PyASTPreprocessState *state)
{
    ...
    _PY_ENSURE_COSTACK_HEADROOM_FOR_FN3_B((PyErr_NoMemory(), 0), int, astfold_pattern, node_, ctx_, state)
    ENTER_RECURSIVE();
    switch (node_->kind) {
