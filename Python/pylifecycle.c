...
Py_InitializeFromConfig(const PyConfig *config)
{
    ...
    if (_PyThreadStack_CallInsideCoroutine(_Py_InitializeFromConfig, &params, NULL)){
...
{
    void *res;
    if (_PyThreadStack_CallInsideCoroutine(__Py_Finalize, runtime, &res)){
        return -1;
    }
                ...
                const PyInterpreterConfig *config, long whence)
{
    ...
    if (_PyThreadStack_CallInsideCoroutine(_new_interpreter, &params, NULL)){
        return PyStatus_NoMemory();
    }
    return params.status;
}
...
Py_EndInterpreter(PyThreadState *tstate)
{
    ...
    bool fail = 
#endif
    _PyThreadStack_CallInsideCoroutine(_Py_EndInterpreter, tstate, NULL);
    assert(!fail);
}
