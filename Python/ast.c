...
validate_constant(PyObject *value)
{
    ...
    if (PyTuple_CheckExact(value) || PyFrozenSet_CheckExact(value)) {
        _PY_ENSURE_COSTACK_HEADROOM_FOR_FN1_B((PyErr_NoMemory(), 0), int, validate_constant, value)
        ENTER_RECURSIVE();
...
validate_expr(expr_ty exp, expr_context_ty ctx)
{
    ...
    int ret = -1;
    _PY_ENSURE_COSTACK_HEADROOM_FOR_FN2_B((PyErr_NoMemory(), 0), int, validate_expr, exp, ctx)
    ENTER_RECURSIVE();
    int check_ctx = 1;
...
validate_pattern(pattern_ty p, int star_ok)
{
    ...
    int ret = -1;
    _PY_ENSURE_COSTACK_HEADROOM_FOR_FN2_B((PyErr_NoMemory(), 0), int, validate_pattern, p, star_ok)
    ENTER_RECURSIVE();
    switch (p->kind) {
...
validate_stmt(stmt_ty stmt)
{
    ...
    int ret = -1;
    _PY_ENSURE_COSTACK_HEADROOM_FOR_FN1_B((PyErr_NoMemory(), 0), int, validate_stmt, stmt)
    ENTER_RECURSIVE();
    switch (stmt->kind) {
