...
PyRun_AnyFileExFlags(FILE *fp, const char *filename, int closeit,
                     PyCompilerFlags *flags)
{
    _PY_ENSURE_COSTACK_FOR_FN4_B(-1, int, PyRun_AnyFileExFlags, fp, filename, closeit,
                        flags)
...
{
    _PY_ENSURE_COSTACK_FOR_FN3_B(-1, int, PyRun_InteractiveLoopFlags, fp, filename, flags)
...
PyRun_SimpleFileExFlags(FILE *fp, const char *filename, int closeit,
                        PyCompilerFlags *flags)
{
    _PY_ENSURE_COSTACK_FOR_FN4_B(-1, int, PyRun_SimpleFileExFlags, fp, filename, closeit,
                            flags)
    PyObject *filename_obj = PyUnicode_DecodeFSDefault(params->filename);
    if (filename_obj == NULL) {
        return (void *)-1;
...
_PyRun_SimpleStringFlagsWithName(const char *command, const char* name, PyCompilerFlags *flags) {
    _PY_ENSURE_COSTACK_FOR_FN3_B(-1, int, _PyRun_SimpleStringFlagsWithName, command, name, flags)
    PyObject *main_module = PyImport_AddModuleRef("__main__");
    if (main_module == NULL) {
        return (void *)-1;
    }
...
{
    _PY_ENSURE_COSTACK_HEADROOM_FOR_FN2_B((PyErr_NoMemory(), -1), int, print_exception, ctx, value)
    PyObject *f = ctx->file;
...
print_exception_cause_and_context(struct exception_print_context *ctx,
                                  PyObject *value)
{
    _PY_ENSURE_COSTACK_HEADROOM_FOR_FN2_B((PyErr_NoMemory(), -1), int, print_exception_cause_and_context, ctx, value)
...
{
    _PY_ENSURE_COSTACK_HEADROOM_FOR_FN2_B((PyErr_NoMemory(), -1), int, print_exception_recursive, ctx, value)
...
_PyRun_StringFlagsWithName(const char *str, PyObject* name, int start,
                           ...
                           int generate_new_source)
{
    _PY_ENSURE_COSTACK_FOR_FN7_B(PyErr_NoMemory(), PyObject *, _PyRun_StringFlagsWithName, str, name, start,
...
PyRun_FileExFlags(FILE *fp, const char *filename, int start, PyObject *globals,
                  PyObject *locals, int closeit, PyCompilerFlags *flags)
{
    _PY_ENSURE_COSTACK_FOR_FN7_B(PyErr_NoMemory(), PyObject *, PyRun_FileExFlags, fp, filename, start, globals,
...
Py_CompileStringObject(const char *str, PyObject *filename, int start,
                       PyCompilerFlags *flags, int optimize)
{
    _PY_ENSURE_COSTACK_FOR_FN5_B(PyErr_NoMemory(), PyObject *, Py_CompileStringObject, str, filename, start,
                        flags, optimize)
    PyCodeObject *co;
    mod_ty mod;
    PyArena *arena = _PyArena_New();
...
Py_CompileStringExFlags(const char *str, const char *filename_str, int start,
                        PyCompilerFlags *flags, int optimize)
{
    _PY_ENSURE_COSTACK_FOR_FN5_B(PyErr_NoMemory(), PyObject *, Py_CompileStringExFlags, str, filename_str, start, flags, optimize)
